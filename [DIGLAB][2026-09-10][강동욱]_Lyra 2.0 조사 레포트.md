...
- 어떤 프레임 하나를 생성할 때 그 프레임 직전 장면의 픽셀들을 3D점으로 띄우고 각 픽셀들을 새 카메라 위치, 자세로 좌표 변환 -> 기존에 보여진 정보들은 보존
#### 광선 정보
![](./이미지/plucker_ray.png)

- d는 카메라 원점에서 픽셀 방향으로의 벡터(카메라의 자세)
- o는 월드 원점에서 카메라 원점으로의 벡터
- m은 두 벡터를 cross dot한 벡터(카메라의 위치가 녹아 있음)
...
| GPU 여러 장 나눠 쓰기         | 안 됨        | 80GB 1장에서도 3D 변환 단계 메모리 부족 issue 있었음(GitHub 이슈에 올라온 사용자 보고)                                                   |
