...
- 포탑이 현재 +175°에 있고, 적이 -175° 방향에 있을 때:
  - turret_rel = -175
  - control_dofs_position(-175) 명령 실행시 포신이 0도로 갔다가 -175도로 회전, 즉 350도를 비효율적으로 돌리는 셈
...
- PD 컨트롤러가 단순히 `target − actual` 차이로 토크 계산(-175 - 175 = -350)
- target은 [-π, +π]로 정규화됐지만 actual(현재 포탑의 각도)은 누적값
- 극단적인 예를 들어 타겟이 0도여도 actual이 720도면 0-720 = -720 만큼 회전할 수 있음

| step | target_rel (wrap 후) | actual (누적) | PD error | 회전 |
|---|---:|---:|---:|---|
| 0→45° | +45 | 0 | +45 | +45 ✓ |
| 45→90° | +90 | +45 | +45 | +45 ✓ |
| 90→135° | +135 | +90 | +45 | +45 ✓ |
| 135→180° | +180 | +135 | +45 | +45 ✓ |
| 180→225° | -135 | +180 | -315 | -315 ✗ |
...
https://github.com/user-attachments/assets/fa32b2f7-1031-422e-960d-acf324556763
